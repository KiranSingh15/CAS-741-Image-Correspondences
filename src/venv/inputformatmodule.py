...
def get_active_methods():
    mthd_img_smoothing, mthd_kp_detection, mthd_kp_description, mthd_ft_match = specParams.get_default_methods()
...
def check_limits(u_sz_kern, u_std_dev, u_fast_thr, u_bin_sz, u_patch_sz):
    ...
    # kernel size
    assert (u_sz_kern > kern_bounds[0]), f"badKernelSize. Kernel must be > 1. (Kernel Size = {u_sz_kern})"
    assert (u_sz_kern < kern_bounds[1]), f"badKernelSize. Kernel must be <= 15. (Kernel Size = {u_sz_kern})"
    assert (u_sz_kern % 2 == 1), f"badKernelSize. Kernel must be odd. (Kernel Size = {u_sz_kern})"
    assert (isinstance(u_sz_kern, int)), f"badKernelSize. Kernel must be an integer. (Kernel Size = {u_sz_kern})"

    # Standard Deviation
    assert (u_std_dev > sd_bounds[0]), f"badStandardDeviation. Standard deviation must be > {sd_bounds[0]} and < {sd_bounds[1]}. (Standard Deviation = {u_std_dev})"
    assert (u_std_dev <= sd_bounds[1]), f"badStandardDeviation. Standard deviation must be > 0 and < 10. (Standard Deviation = {u_std_dev})"

    # Pixel Intensity
    assert (u_fast_thr >= fast_bounds[0]), f"badFASTThreshold. Threshold must be >= {fast_bounds[0]} and <= {fast_bounds[1]}. (Threshold = {u_fast_thr})"
    assert (u_fast_thr <= fast_bounds[1]), f"badFASTThreshold. Threshold must be >= {fast_bounds[0]} and <= {fast_bounds[1]}. (Threshold = {u_fast_thr})"
    assert (isinstance(u_fast_thr, int)), f"badFASTThreshold. Threshold must be an integer. (Kernel Size = {u_fast_thr})"

    # Bin Size
    assert (u_bin_sz>= bin_bounds[0]), f"badBinSize. Bin Size must be >= {bin_bounds[0]} and <= {bin_bounds[1]}. (Bin size = {u_bin_sz})"
    assert (u_bin_sz <= bin_bounds[1]), f"badBinSize. Bin Size must be = {bin_bounds[0]} and <= {bin_bounds[1]}. (Bin size = {u_bin_sz})"
    assert (isinstance(u_bin_sz, int)), f"badBinSize. Bin Size must be an integer. (Bin Size = {u_bin_sz})"

    # Patch Size
    assert (u_patch_sz >= patch_sz[0]), f"badPatchSize. Patch size must be >= {patch_sz[0]} and <= {patch_sz[1]}. (Patch size = {u_patch_sz})"
    assert (u_patch_sz <= patch_sz[1]), f"badPatchSize. Patch size must be >= {patch_sz[0]} and <= {patch_sz[1]}. (Patch size = {u_patch_sz})"
    assert (isinstance(u_patch_sz, int)), f"badPatchSize. Patch Size must be an integer. (Patch Size = {u_patch_sz})"
