...
def test_output_keypoints_variable_size(tmp_path, num_points):
    ...
    output_file = (
        tmp_path / "Outputs" / outmod.keypoint_folder_nm / f"{image_id}_kp.csv"
    )
...
def test_output_descriptors_variable_size(tmp_path, num_points):
    ...
    output_file = (
        tmp_path / "Outputs" / outmod.descriptor_folder_nm / f"{image_id}_fd.csv"
    )
    assert output_file.exists()
...
def test_output_matches_variable_size(tmp_path, num_matches):
    ...
    outmod.output_matches(
        query_id, train_id, matches, kp1, kp2, tmp_path, outmod.matches_folder_nm
    )
